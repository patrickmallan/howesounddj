...
function getAutoReplyPlainText(): string {
  return [
    ...
    "",
    "In the meantime, you can Book a Sound Check or Check Availability from the same page here:",
    "",
...
<p>In the meantime, you can Book a Sound Check or Check Availability from the same page here:</p>
    ...
    return NextResponse.json<ContactApiResponse>(
      {
        success: false,
        message:
          "Delivery is not fully configured yet. Please try again later. You can still book a Sound Check on howesounddj.com/contact.",
      },
      { status: 503 }
    );
  }
