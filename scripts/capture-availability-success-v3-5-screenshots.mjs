#!/usr/bin/env node
/**
 * Screenshot capture for Availability Success V3.5 final polish and design freeze.
 */
...
const OUT = join(process.cwd(), "docs/handoff/screenshots/availability-success-v3-5");
...
async function captureEditDateRestored(browser) {
  const context = await browser.newContext({
...
async function main() {
  ...
  try {
    ...
    await captureCompact(browser, 1512, 982, "compact-available-1512x982.png");
    ...
    await captureContactFull(browser, 390, 844, "contact-full-available-390.png", true);
    await captureEditDateRestored(browser);
    await captureReducedMotion(browser);
    await captureZoom200(browser);
    console.log(`V3.5 screenshots saved to ${OUT}`);
  } finally {
    await browser.close();
  }
}
