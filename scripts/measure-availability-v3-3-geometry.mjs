#!/usr/bin/env node
/**
 * Geometry certification for Availability Success V3.3 composition cohesion.
 */
...
const OUT = join(OUT_DIR, "implementation-v3-3-geometry-proof.json");
...
async function measureViewport(browser, vp) {
  ...
  const metrics = await panel.evaluate((el) => {
    ...
    const proofContext = success?.querySelector('[data-availability-role="proof-context"]');
    ...
    const narrativeRect = success
      ?.querySelector('[data-availability-role="narrative"]')
    ...
    const footerBorderVisible =
      footerStyles &&
      footerStyles.borderTopWidth !== "0px" &&
      parseFloat(footerStyles.borderTopColor?.split(",")[3] ?? "0") > 0.05;

    const headingLines = heading?.querySelectorAll("span").length ?? 0;

    const ctaSupportLeftAligned =
      ctaSupportRect && narrativeRect
        ? Math.abs(ctaSupportRect.left - narrativeRect.left) < 4
        : false;
    ...
    return {
      ...
      proofContextAbsent: !proofContext,
      ...
      stephenVisible: attribution
        ...
        : false,
      footerDividerVisible: Boolean(footerBorderVisible),
  ...
  try {
    ...
    const summary = {
      ...
      pass:
        ...
        results.every((r) => r.metrics.proofContextAbsent === true) &&
        results.every((r) => r.metrics.footerDividerVisible === false) &&
    ...
    console.log(`V3.3 geometry proof written to ${OUT}`);
