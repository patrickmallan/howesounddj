#!/usr/bin/env node
/**
 * Geometry certification for Availability Success V3.2 visual hierarchy.
 * Outputs research/availability-success-v3/implementation-v3-2-geometry-proof.json
 */
...
const OUT = join(OUT_DIR, "implementation-v3-2-geometry-proof.json");
...
async function measureViewport(browser, vp) {
  ...
  const metrics = await panel.evaluate((el) => {
    ...
    const venueEl = success?.querySelector('[data-availability-role="attribution"] span:last-child');
    const contextLabel = success?.querySelector('[data-availability-role="proof-context"]');
    const ctaSupport = success?.querySelector('[data-availability-role="cta-support"]');
    const quote = success?.querySelector('[data-availability-role="testimonial"]');
    ...
    const quoteWeight = quote ? window.getComputedStyle(quote).fontWeight : null;
    ...
    return {
      ...
      contextLabelVisible: contextLabel ? contextLabel.getBoundingClientRect().height > 0 : false,
      attributionVisible: success
        ? Boolean(success.querySelector('[data-availability-role="attribution"]'))
        : false,
      quoteFontWeight: quoteWeight,
  ...
  try {
    ...
    const summary = {
      ...
      pass:
        ...
        results.every((r) => r.metrics.venueText === "Sea to Sky Gondola") &&
        results.every((r) => r.metrics.ctaWraps === false),
    };
    writeFileSync(OUT, JSON.stringify(summary, null, 2));
    console.log(`V3.2 geometry proof written to ${OUT}`);
