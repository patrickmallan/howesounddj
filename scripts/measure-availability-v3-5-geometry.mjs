#!/usr/bin/env node
/**
 * Geometry certification for Availability Success V3.5 final polish and design freeze.
 */
...
const OUT = join(OUT_DIR, "implementation-v3-5-geometry-proof.json");
...
async function measureViewport(browser, vp) {
  ...
  const metrics = await panel.evaluate(
    (el, { excerpt, venueContext, openQuote, closeQuote }) => {
      ...
      const quoteAlignment = testimonialStyles?.textAlign ?? null;
      const quoteJustified = quoteAlignment === "justify";

      const ctaSupportStyles = ctaSupport ? window.getComputedStyle(ctaSupport) : null;
      const ctaSupportCentered = ctaSupportStyles?.textAlign === "center";
      const ctaSupportLineHeight = ctaSupport ? ctaSupport.getClientRects().length : 0;
      ...
      const footerObscuresContent =
        bodyScroll &&
        footerRect &&
        bodyRect &&
        footerRect.top < bodyRect.bottom - 2 &&
      ...
      return {
        ...
        stephenVisible: attribution
          ...
          : false,
        excerptText,
        quotationMarksVisible:
          excerptText.startsWith(openQuote) && excerptText.endsWith(closeQuote),
        excerptExactBetweenQuotes:
          excerptText === `${openQuote}${excerpt}${closeQuote}`,
        bridgeAbsent,
        testimonialItalic,
        quoteAlignment,
        quoteJustified,
        ctaSupportCentered,
        ctaSupportLineCount: ctaSupportLineHeight,
        headlineLineCount: headingLines,
        headlineHeight: headingRect ? Math.round(headingRect.height) : null,
        testimonialLeft: testimonialRect ? Math.round(testimonialRect.left) : null,
        bodyScrollRequired: bodyScroll
          ? bodyScroll.scrollHeight > bodyScroll.clientHeight
          : false,
        footerObscuresContent: Boolean(footerObscuresContent),
      };
    },
    {
      excerpt: STEPHEN_EXCERPT,
      venueContext: VENUE_CONTEXT,
      openQuote: OPENING_QUOTE,
      closeQuote: CLOSING_QUOTE,
    },
  );
...
async function main() {
  ...
  try {
    ...
    const summary = {
      ...
      baseUrl: BASE,
      variant: "human_connection_v3_5",
      testimonialWeight: "font-medium italic (Geist Sans native)",
      viewports: results,
      pass:
        ...
        results.every((r) => r.metrics.quotationMarksVisible === true) &&
        results.every((r) => r.metrics.excerptExactBetweenQuotes === true) &&
        results.every((r) => r.metrics.bridgeAbsent === true) &&
        results.every((r) => r.metrics.testimonialItalic === true) &&
        results.every((r) => r.metrics.quoteAlignment === "left" || r.metrics.quoteAlignment === "start") &&
        results.every((r) => r.metrics.quoteJustified === false) &&
        results.every((r) => r.metrics.ctaSupportCentered === true) &&
        results.every((r) => r.metrics.ctaSupportLineCount <= 2) &&
        results.every((r) => r.metrics.ctaWraps === false) &&
        results.every((r) => r.metrics.footerObscuresContent === false) &&
        results.every((r) => r.metrics.headlineLineCount === 2),
    };
    writeFileSync(OUT, JSON.stringify(summary, null, 2));
    console.log(`V3.5 geometry proof written to ${OUT}`);
